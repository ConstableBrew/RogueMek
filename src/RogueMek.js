...
function createHexMap(){
	...
	row8[6] = new WaterHex();	row8[6].elevation = -2;
	row8[7] = new WaterHex();
	row8[8] = new WaterHex();
	...
	row10[7] = new WaterHex();	row10[7].elevation = -2;
...
function commitMove(mech, newXY, points, jumping){
	...
	// calculate total heatGen from movement for each step
	for(var i=0; i<mech.heldMoves.length; i++){
		...
		if(prevMove.jumping){
			// add jump heat (instead of minimum 3 heat jump rule we'll just use a flat 2 heat per jump point used)
			mech.heatGen += (prevMove.points * 2) / turnsPerRound;
		}
		else{
			var heatMultiplier = isMechRunning(mech) ? 2: 1;
			mech.heatGen += (prevMove.points * heatMultiplier) / turnsPerRound;
		}
	}
	...
	if(jumping){
		...
		if(points > 0){
			mech.heatGen += (points * 2) / turnsPerRound;
		}
		...
		if(points > 0){
			var heatMultiplier = isMechRunning(mech) ? 2: 1;
			mech.heatGen += (points * heatMultiplier) / turnsPerRound;
		}
